...
- `ci.yml` runs `go test`, validates `tinx release` with `sourceplane/tinx-release-action@v2`, and smoke-tests both transient providers and a workspace manifest through `sourceplane/tinx-action@v2`.
- `release.yml` publishes the provider with `sourceplane/tinx-release-action@v2`.
